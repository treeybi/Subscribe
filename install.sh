...
install_v2ray() {
    ...
    wget https://github.com/v2ray/v2ray-core/releases/download/v4.23.1/v2ray-linux-64.zip && bash <(curl -L -s https://raw.githubusercontent.com/Jacobabc123/Subscribe/master/go.sh) --local /root/v2ray-linux-64.zip
    if [[ $? -ne 0 ]]; then
...
install_v2-ui() {
    ...
    last_version=$(curl -Ls "api.github.com/repos/sprov065/v2-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo -e "检测到v2-ui最新版本：${last_version}，开始安装"
    wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://github.com/sprov065/v2-ui/releases/download/5.2.1/v2-ui-linux.tar.gz
    if [[ $? -ne 0 ]]; then
